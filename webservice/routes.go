...
func getAgentsList(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := tx.Find(&agents).Limit(100).Error; err != nil {
		...
		for i := range agents {
			db.Find(&agents[i].Monitors, "agent_id = ?", agents[i].Id)
			db.Find(&agents[i].Disks, "agent_id = ?", agents[i].Id)
		}
...
func postChangePassword(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		confirmPwd := c.PostForm("confirmPassword")
		currentPwd := c.PostForm("currentPassword")
...
func postRemoveUser(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := db.Preload("Files").Where("guid = ?", userid).
			First(&currentUser).Error; err != nil {
			c.String(404, "Not found")
			return
		}

		if err := db.Delete(&models.User{}, "guid = ?", userid).Error; err != nil {
			c.String(404, "Not found")
			return
		}
...
func postRemoveAgent(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		db.Where("agent_id = ?", toRemove.Id).Delete(&models.MonitorEntry{})
		db.Where("agent_id = ?", toRemove.Id).Delete(&models.DiskEntry{})
		c.Redirect(302, "/agent_list")
